...
public class DelDepartment extends JPanel implements ActionListener {
	...
	Statement st;
	// 创建组件对象
	JLabel jLabel1 = new JLabel("请输入待删除的部门系号:");
	JLabel jLabel2 = new JLabel("(2位数,如:05)");
	...
	public void actionPerformed(ActionEvent e) {
		...
		if (e.getSource() == jButton1) {
			...
			try {
				...
				// 处理结果集:逐条显示结果集中的记录
				// 此处没有使用while，因为系号是唯一的，而系名不是，所以在InquireOnNamePanel.java里使用了while
				if (rs2.next()) {
					jTextArea1.append("系号:" + rs2.getString("XiId") + "\n系名:" 
							+ rs2.getString("XiName")+ "\n系主任:" + rs2.getString("xiTeacher"));
				} else
		...
		// 要求用户确认删除
			...
			try {
				...
				// 清空输入系号的文本行
				jTextField1.setText("");
				// 每次确认删除之后清空原来要删除的部门系信息，否则，下次删除还会有上次刪除的部门系信息
				jTextArea1.setText("");
				jScrollPane1.setVisible(false);
				jButton2.setVisible(false);
